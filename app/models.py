...
class Recipe(db.Model):
    ...
    # Foreign key to User
    user_id = db.Column(db.String(36), db.ForeignKey("users.id", name="fk_recipes_user_id_users"), nullable=True)
    ...
    ingredients = db.Column(db.Text, nullable=False)  # JSON string: {"ingredient": "description"}
    ...
    original_id = db.Column(db.String, db.ForeignKey('recipes.id', name="fk_recipes_original_id_recipes_id"), nullable=True)
    ...
    def recipe_already_copied(self, user_id):
        ...
        return Recipe.query.filter_by(user_id=user_id, original_id=self.original_id).first() is not None
    ...
    def search_all_attributes(search_string, user_id=None):
        ...
        for recipe in recipes:
            ...
            if any(search_lower in key.lower() or search_lower in value.lower() 
                   for key, value in ingredients.items()):
                matching.append(recipe)
                continue
            
            # Search in instructions
            if any(search_lower in instruction.lower() for instruction in recipe.instructions_list):
                matching.append(recipe)
                continue
